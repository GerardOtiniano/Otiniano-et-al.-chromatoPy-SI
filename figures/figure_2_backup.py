...
smaller, larger, df = user_comparison(df)
...
# Panel A
smaller_ax = ax0.scatter(smaller['chromatopy_pa'], smaller['perc_diff'], c='k', marker='d', 
                         ec='k', s=70, alpha=0.5, label='Difference < uncertainty')
larger_ax = ax0.scatter(larger['chromatopy_pa'], larger['perc_diff'], c='red', marker='d',
                        ec='k', s=70, alpha=0.5, label='Difference > uncertainty')
ax0.axhline(0, c='k')
...
ax1.scatter(filt_sm['chromatopy_pa'], filt_sm['perc_diff'], c='k', marker='d', ec='k', s=70, alpha=0.5)
ax1.scatter(filt_lg['chromatopy_pa'], filt_lg['perc_diff'], c='red', marker='d', ec='k', s=70, alpha=0.5)
...
# Panel D
thresh, below, above = threshold_outliers(results) # get data above and below outlier threshold (2-sigma)
ax3.set_title('Uncertainty')
ax3.scatter(below["chromatopy_pa"], below.rel_err*100, marker='s', s=70, c="k", ec="k", alpha=0.5, label="Uncertainty within 95$^{th}$ percentile")
ax3.scatter(above["chromatopy_pa"], above.rel_err*100, marker='s',s=70, c="red", ec="k", alpha=0.5, label="Uncertainty above 95$^{th}$ percentile")
...
ax6.scatter(clean_df['chromatopy_ra'], clean_df['user-chromatopy'], c='k', ec='k', alpha=0.5, s=70)
ax6.axhline(0, c='k')
